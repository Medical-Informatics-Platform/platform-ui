...
export class ExperimentStudioService {
  ...
  // One-time migration: when algorithmY/algorithmX were never persisted, treat the
  // legacy variables/covariates split as the y/x roles and unify them into the pool.
  private migrateLegacyRoleState(): void {
    const hasY = this.sessionStorage.getItem('algorithmY') !== null;
    const hasX = this.sessionStorage.getItem('algorithmX') !== null;
    if (hasY && hasX) return;

    const legacyVars = this.selectedVariablesSignal();
    // One-time migration reads the legacy covariates list straight from storage.
    const legacyCovs = this.sessionStorage.getItem<any[]>('selectedCovariates') || [];
    if (hasY) {
    ...
    // The legacy covariate list is fully replaced by algorithmX.
  ...
  /**
   * Normalize a UI-store categorical_column_creator value (one object for a
   * single column, or an array for two+) into an ordered array of configs.
   */
  static categoricalCreators(value: unknown): Record<string, unknown>[] {
    if (!value) return [];
  ...
  /** Every applied categorical_column_creator config as an array (legacy single object included). */
  appliedCategoricalCreators(): Record<string, unknown>[] {
    return ExperimentStudioService.categoricalCreators(
      this.appliedPreprocessingConfig()?.['categorical_column_creator']
    );
  }
   ...
   */
  setTransformationPreprocessing(config: Record<string, unknown> | Record<string, unknown>[] | null): void {
    ...
    const creators = ExperimentStudioService.categoricalCreators(config);
    if (creators.length === 1) {
      next['categorical_column_creator'] = creators[0];
    } else if (creators.length > 1) {
      next['categorical_column_creator'] = creators;
    } else {
