...
export class AlgorithmPanelComponent {
  ...
  getRoleRequirement(field: any, label: string, algorithmName?: string | null): string | null {
    ...
    const minCount = resolveInputMinCount(field, label === 'Outcome' ? 'y' : 'x', algorithmName);
    if (label === 'Predictor' && minCount < 1) {
      return null;
    }

    const count = formatInputCountRange(minCount, resolveInputMaxCount(field, label === 'Outcome' ? 'y' : 'x', algorithmName));
  ...
  getVariableRequirement(algo?: any): string | null {
    ...
    return this.getRoleRequirement(target?.inputdata?.y, 'Outcome', target?.name);
  }

  getCovariateRequirement(algo?: any): string | null {
    const target = algo || this.tooltipData();
    const field = target?.inputdata?.x;
    return this.getRoleRequirement(field, 'Predictor', target?.name);
  }
