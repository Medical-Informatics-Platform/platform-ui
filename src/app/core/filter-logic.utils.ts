...
/**
 * Unary rules must carry their value key. The reference filter client (the notebook)
 * sends `"value": null` for `is null` / `is not null`, and a payload without the key is
 * rejected by the engine with an opaque failure — so a tree read back from a stored
 * experiment is repaired on the way in. Group nodes, bare rules, arrays and non-objects
 * are all returned untouched; nothing here invents a tree.
 */
export function normalizeFilterLogicForBackend<T>(logic: T): T {
  if (Array.isArray(logic)) {
    return logic.map((child) => normalizeFilterLogicForBackend(child)) as unknown as T;
  }
  if (!logic || typeof logic !== 'object') {
    return logic;
  }
  const node = logic as Record<string, any>;
  if (Array.isArray(node['rules'])) {
    return { ...node, rules: normalizeFilterLogicForBackend(node['rules']) } as unknown as T;
  }
  return node['value'] === undefined && isUnaryFilterOperator(String(node['operator'] ?? ''))
    ? { ...node, value: null } as unknown as T
    : logic;
}
